...
### ADR-030: Every Required Address Field Offers Selectable Saved Places
**Decision**: Add selectable saved/recent places to all required address fields so participants can tap instead of free-typing: the ride destination (focus-triggered suggestion dropdown), the Courier sender (saved list shown while empty), and the Eats G3 delivery (saved list shown while empty). Selection fills the field and clears the validation error. Auto-populate conditions (G2 sender, G4 delivery) still pre-fill the field, so the saved lists only surface in the manual-entry conditions.
**Date**: 2026-06-24
**Rationale**: Strict required-text validation with no affordance blocked participants (e.g. typing "1386" then being unable to continue). Saved/recent places are a normal feature of any ride/delivery app and mirror the existing Courier recipient saved-address picker, so they are condition-neutral and do not alter the auto-populate manipulation (treatment still pre-fills the exact trip address + nudges via banner). Test ids: `destination-suggestions` / `destination-suggestion-{id}`, `sender-saved-places` / `sender-saved-{id}`, `delivery-saved-places` / `delivery-saved-{id}`.
